(() => {
  ...
  function d() {
    const t = document.getElementById("time-now"),
      e = new Date(),
      o = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][e.getDay()],
      a = (e.getMonth(), e.getDate());
    let n = e.getHours(),
      s = e.getMinutes();
    const c = n >= 12 ? "PM" : "AM";
    (n = n % 12 || 12), (s = s < 10 ? "0" + s : s);
    const i = `${o} ${a} ${n}:${s} ${c}`;
    t.textContent = i;
  }
  new (class {
    ...
    d(),
    setInterval(d, 3e4);
  const h = document.querySelectorAll(".pl-func"),
    u = document.querySelectorAll(".pr-func");
  let m;
  function g() {
    (m = window.innerWidth < 1630 ? 15 : (window.innerWidth - 1600) / 2),
      h.length &&
        h.forEach((t) => {
          t.style.paddingLeft = m + "px";
        }),
      u.length &&
        u.forEach((t) => {
          t.style.paddingRight = m + "px";
        });
  }
  g(),
